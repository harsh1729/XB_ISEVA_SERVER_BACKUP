...
<script type="text/javascript">
	$(function(){
        ...
        $('#createCategoryForm').on('submit',function(event) {
			...
			postData.push({"name":"cityid","value":advtcityselectid});
			...
			else
			{
				...
				$.ajax({                
						...
						dataType:'json',
						//url:"<?=base_url('admin_requests/news/get_news')?>",
						url:xb_global_namespace.baseurl+"admin_requests/epaper_users/add",
						data:postData,
						success:function(datarecieved,textStatus,jqXHR)
						{
							console.log(datarecieved);
    ...
    ADVERTISEMENT_NAMESPACE = {};
		ADVERTISEMENT_NAMESPACE.getDomString = function(id,name,address,contact,username,isenable,imagedata,password){
			var enablecheck = "";
    ...
    setDisablenable = function(id){
        	...
        	$.ajax({                
					...
					type:'POST',
					dataType:'json',
					url:xb_global_namespace.baseurl+"admin_requests/epaper_users/setactive",
					data:{
						"id":id,
						"isenable":isenable
...
//ADVERTISEMENT_NAMESPACE.loadAd();
				ADVERTISEMENT_NAMESPACE.loadAd = function(){
									...
									$.ajax({                
											...
											type:'POST',
											dataType:'json',
											url:xb_global_namespace.baseurl+"admin_requests/epaper_users/get",
											data:{
												"cityid":cityid,
											},
											success:function(datarecieved,textStatus,jqXHR)
											{
												...
												if(datarecieved['status'] == 'login')
												{
													...
													$.each(data, function(index, val) {
														//console.log(val);
														$(ADVERTISEMENT_NAMESPACE.getDomString(val.id,val.name,val.address,val.contact,val.username,val.isenable,val.image,val.password)).hide().appendTo('#newsContainerColumn').fadeIn('1000', function() {});
													});
													$('.bootstraptogglebtn').bootstrapToggle();
