...
<script type="text/javascript">
	$(function(){
        ...
        $('#createCategoryForm').on('submit',function(event) {
			...
			postData.push({"name":"bextraid","value":'7'});
			postData.push({"name":"userid","value":'1'});
			postData.push({"name":"isbusiness","value":'0'});
			postData.push({"name":"catid","value":'0'});
			postData.push({"name":"advtcityselectid","value":'0'});
    ...
    ADVERTISEMENT_NAMESPACE = {};
		ADVERTISEMENT_NAMESPACE.getDomString = function(id,heading,content,expiredate,publishdate,isenable,imagedata){
			var enablecheck = "";
...
//ADVERTISEMENT_NAMESPACE.loadAd();
				ADVERTISEMENT_NAMESPACE.loadAd = function(){
									...
									$.ajax({                
											...
											type:'POST',
											dataType:'json',
											url:xb_global_namespace.baseurl+"admin_requests/businessextra/getgovtjobs",
											data:{
											   "userid":1,
											   "isbusiness":0
											},
											success:function(datarecieved,textStatus,jqXHR)
											{
												...
												if(datarecieved['status'] == 'login')
												{
													...
													$.each(data, function(index, val) {
														//console.log(val);
														$(ADVERTISEMENT_NAMESPACE.getDomString(val.id,val.heading,val.content,val.expirydatetime,val.publishdatetime,val.isnable,val.image)).hide().appendTo('#newsContainerColumn').fadeIn('1000', function() {});
													});
													$('.bootstraptogglebtn').bootstrapToggle();
