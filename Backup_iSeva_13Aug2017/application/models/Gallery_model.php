...
class Gallery_model extends CI_Model{
	...
	public function getResumedata($imageid)
	{
		$this->db->where($this->KEY_ITEM_ID,$imageid);
		$query = $this->db->get($this->TABLE_NAME_GALLERY);
		$item = array();
		$item['imageurl'] = ""; //Added by Harsh
		foreach ($query->result() as $row) {

			$item = $this->parseResumeData($row->id,$row->name,$row->datetime,$row->size,$row->width,$row->height);
	...
	public function parseResumeData($id,$name,$timestamp,$size,$width,$height)
	{
		$item = array();
		...
		$item['yeardir'] = $YEAR;
		$item['monthdir'] = $MONTH;
		$item['imageurl'] = base_url("publicuploadsresume".DIRECTORY_SEPARATOR.$YEAR.DIRECTORY_SEPARATOR.$MONTH.DIRECTORY_SEPARATOR.$name);
		$item['thumburl'] = base_url("publicuploadsresume".DIRECTORY_SEPARATOR.$YEAR.DIRECTORY_SEPARATOR.$MONTH.DIRECTORY_SEPARATOR."thumb_".$name);
		$item['datetime'] = $datetime;
		$item['size'] = $size;
		$item['width'] = $width;
		$item['height'] = $height;
   ...
   function upoadresume($upload_path,$datetime,$relativepath,$userid,$keys){
   ...
   if (!empty($_FILES))
		{
			...
			foreach ($arrayKeys as $key)
			{
				...
				$temp = array(
						"id" => $insertid,
						"name" => $timedImgName,
						"url" => base_url($relativepath.DIRECTORY_SEPARATOR.$timedImgName)
						);
				$imagename[$key] = $temp;
				//array_push($imagename,$temp);
			} 
