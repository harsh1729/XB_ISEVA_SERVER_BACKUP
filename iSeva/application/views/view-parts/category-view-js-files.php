...
<script type="text/javascript">
	$(function(){
		...
		$('#createCategoryForm').on('submit',function(event) {
			...
			postData.push({"name":"parentid","value":"0"})
		...
		CATEGORY_NAMESPACE = {};
		CATEGORY_NAMESPACE.getCategoryDomString = function(catid,name,parentid,sortorder,isenable,imagedata,issubcate){
			var enablecheck = "";
			...
			if(isenable == "1")
				enablecheck = " checked ";
			if(issubcate=='1'){
			domString = '<div class="well well-sm sortableitem" id="category-'+catid+'">'+
								'<div class="row">'+
									'<div class="col-md-4">'+
			...
			$.ajax({                
						...
						success:function(datarecieved,textStatus,jqXHR)
						{
							...
							if(datarecieved['status'] == 'login')
							{
								var data = datarecieved['data'];
								$.each(data, function(index, val) {
									$(CATEGORY_NAMESPACE.getCategoryDomString(val.catid,val.name,val.parentid,val.sortorder,val.isenable,val.image,val.issubcate)).hide().appendTo('#newsContainerColumn').fadeIn('slow',function() {});
								});
